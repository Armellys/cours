...
void cesar_crypt(int decallage, char * texte, char* chiffre)
{
	int i=0;
	char c;
// la variable c va servir ‡ gerer les diffÈrents cas de caractËre dans un texte, je ne gËre pas les caractËres spÈciaux
	while (texte[i] != '\0'){
		c = texte[i];
		if(c>='A' && c<='Z')
...
void viginere_crypt(char * key, char * texte, char* chiffre)
{
	...
	int clef;
	// je gËre plusieur type de clef possible
	while (texte[i] != '\0'){
		c = texte[i];
		clef = key[i%taille]; 
		if( clef>='A' && clef<='Z')
...
void des_decrypt(char * key, char * texte, char* chiffre, int size)
{
	...
	for(i=0;i<size*8;i+=8){
		...
		// on met le bloc dÈchiffrÈ dans le rÈsultat
		for (j= 0; j < 8; j++)
		{
			chiffre[i+j] = blocR[j];
		}
...
void tripledes_crypt(char * key1, char * key2, char * texte, char* chiffre,int size)
{
	...
	for(i=0;i<size*8;i+=8){
		...
		//on chiffre en 3DES ( 1 chiffrement, 1 dÈchiffrement puis 1 chiffrement)
...
void tripledes_decrypt(char* key1, char* key2, char* texte, char* chiffre, int size)
{
	...
	for(i=0;i<size*8;i+=8){
		...
		des_decipher(blocT, blocB, key1); // On dÈchiffre en faisant l'inverse du chiffrement 3DES
